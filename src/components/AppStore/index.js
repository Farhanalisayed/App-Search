...
class AppStore extends Component {
  ...
  activatedTab = tabId => {
    this.setState({tab: tabsList.filter(each => each.tabId === tabId)})
    this.setState({apps: appsList.filter(each => each.category === tabId)})
  }

  render() {
    const {tab, apps, searchInput} = this.state
    const searchResult = apps.filter(each =>
